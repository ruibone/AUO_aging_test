combinemean <- read.csv("C:/Users/agron_thesis/資料處理紀錄/combinemean.csv", sep = ",")
combinemean <- combinemean[,-1]
colnames(combinemean)[1] <- "accession"

PIref <- read.csv("C:/Users/agron_thesis/資料處理紀錄/crawler_pimpinellifolium_list.csv", sep = ",")
...
full <- merge(x = PIref[,c(1,4)], y = combinemean[,c(1,50:55)], by = "accession", all = TRUE)
...
write.csv(full, "C:/Users/Darui Yen/OneDrive/桌面/all_mean_sd.csv")
